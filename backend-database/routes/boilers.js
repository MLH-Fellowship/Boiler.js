...
router.route("/").get((req, res) => {
    Boiler.find().then((boilers) => res.json(boilers)).catch((err) => res.status(400).json("error: " + err));
});
...
router.route("/").post((req, res) => {
    ...
    newBoiler.save().then((boiler) => res.json(boiler._id)).catch((err) => res.status(400).json("error: " + err));
});

// GET existing boiler
router.route("/:id").get((req, res) => {
    Boiler.findById(req.params.id).then((boiler) => res.json(boiler)).catch((err) => res.status(400).json("error: " + err));
});

// DELETE existing boiler
router.route("/:id").delete((req, res) => {
    Boiler.findById(req.params.id).then(() => res.json("boiler deleted!")).catch((err) => res.status(400).json("error: " + err));
});
...
router.route("/query/:input/").get((req, res) => {
    ...
    if (query === "") {
        ...
        }).then((boiler) => res.json(boiler)).catch((err) => res.status(400).json("error: " + err));
    }
});
...
router.post("/:id/image", upload.array("image", 1), (req, res) => {
    ...
    Boiler.findById(req.params.id).then((boiler) => {
        ...
        boiler.save().then((boiler) => res.json(boiler)).catch((err) => res.status(400).json(err));
    });
});
